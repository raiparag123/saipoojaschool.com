...
if(isset($_REQUEST["edit_sname"]))	{
	...
	if(isset($_POST["subj_struct"]))	{ //fetch table data for class fee structure.
		...
		$medm_id=$_POST['medm_id'];
								...
								// change080320#5 $stmt = $mysqli->prepare("select a.subject_name,b.class_subject_id from subject_master a,class_subject_master b  where a.subject_id=b.subject_id and b.class_id=(select class_id from class_master where class_name=? and medium=?) and fy_id=?;");
								$stmt = $mysqli->prepare("select a.subject_name,b.class_subject_id from subject_master a,class_subject_master b  where b.is_delete=1 and a.subject_id=b.subject_id and b.class_id=(select class_id from class_master where class_name=? and medium=?) and b.fy_id=?");
...
if(isset($_REQUEST["del_class_sub"]))	{
	...
	// change080320#4old $stmt = $mysqli->prepare("delete from class_subject_master WHERE class_subject_id=?");
	$stmt = $mysqli->prepare("update   class_subject_master set is_delete = 0 WHERE class_subject_id=?");
