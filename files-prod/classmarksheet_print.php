...
?>
			...
			<div class="page-content-wrapper">
					...
					<div class="page-bar">
						...
						<div class="page-title-breadcrumb">
							...
							<div class="text-right">		
									...
									<!--<button  class="btn btn-default btn-outline" type="button"> <span><i class="fa fa-print"></i><a href="marksheet_edit.php?cs_id=Modify</a></span> </button>-->
							</div>
						</div>
				...
				<?php 
				$flag=0;
					$stmt1 = $mysqli->prepare("select b.cs_id from class_master a,class_stud_master b where a.class_id=b.class_id and a.class_id=? and b.fy_id=?");
					$stmt1->bind_param("dd",$class_id,$fy_id);
					$result1 = $stmt1->execute();
					$stmt1->store_result();
					$stmt1->bind_result($cs_id);		
					$numberofrows = $stmt1->num_rows;
					if($numberofrows ==0 ){
							header('Location: classmarksheet_setup.php?response=1');
					}
					while($data1 = $stmt1->fetch()){

					

					$stmt = $mysqli->prepare("SELECT regno,first_name,mother_name,father_name,dob,class_name FROM stud_regist_master a,class_stud_master b,class_master c,student_term_master d where b.class_id=c.class_id and a.registration_id=b.student_id and b.cs_id=? and b.cs_id=d.cs_id");
					$stmt->bind_param("d",$cs_id);
					$result = $stmt->execute();
					$stmt->store_result();
					$stmt->bind_result($regno,$name,$mname,$fname,$dob,$class_name);		
					$numberofrows1 = $stmt->num_rows;
							...
							<div class="col-md-12">
								<div class="row" style="border: 2px solid #000">
									...
									<div class="col-md-12 text-left" style="margin-top: 10px;">
										School Reopens on 2nd April 2019
									</div>
								</div>
		...
		if($flag==0)
				header('Location: classmarksheet_setup.php?response=2');
			?>
