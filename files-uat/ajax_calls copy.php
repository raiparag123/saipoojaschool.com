...
$medm_id=$_POST['medm_id'];
						...
						*/
						$stmt = $mysqli->prepare("select a.fee_id,b.fee_struct_id,b.fee_amount,c.fee_name,c.fee_name_id from fee_master a,fee_struct_master b,fee_name_master c where a.fee_id=b.fee_id and b.fee_name_id=c.fee_name_id and b.is_delete = 1 and  a.fy_id= ? and a.class_id=(select class_id from class_master where class_name=? and medium=?)");
						$stmt->bind_param("dss",$fy_id,$class_id,$medm_id);
						$result = $stmt->execute();
						$stmt->store_result();
	...
	if(isset($_REQUEST["edit_classfee"]) ||    isset($_REQUEST["add_struct"]) )	{
		...
		else		
		{
			...
			$stmt = $mysqli->prepare("select a.fee_id from fee_master a,class_master b where  a.class_id=b.class_id and b.class_name=? and b.medium=? and  a.fy_id= ?");
			$stmt->bind_param("ssd",$cls_id,$mdmid,$fy_id);
			$result = $stmt->execute();
			$stmt->store_result();
		...
		header("Location:print_fees.php");
			...
			if(isset($_POST["showdcr"]))
			{
				...
				$dates=date('Y-m-d',strtotime($showdcr));
									$stmt = $mysqli->prepare("SELECT e.father_name,a.cs_id,e.regno,e.first_name,d.class_name,f.month_name,sum(b.fee_amount),g.fy_name,f.month_id FROM stud_fee_master a,stud_fee_log_master b,class_stud_master c,class_master d,stud_regist_master e,month_master f,fy_master g WHERE a.month_id=f.month_id and g.fy_id=c.fy_id and   a.stud_fee_id=b.stud_fee_id and c.cs_id=a.cs_id and d.class_id=c.class_id and e.registration_id=c.student_id and a.datetimes like '%$dates%' GROUP by c.cs_id");
									$result = $stmt->execute();
									$stmt->store_result();
